/**
 * Eight Off — a FreeCell cousin. Pure rules + presentation, added without
 * touching the engine or the render shell.
 *
 * 8 tableau columns (all cards face-up, 6 each), 8 free cells (each holds one
 * card; 4 are filled at the deal), 4 foundations (build up A→K by suit).
 * Tableau builds DOWN by the SAME SUIT; only a King may move to an empty column.
 * A "supermove" of a sequence is allowed up to
 * (freeCells + 1) · 2^(emptyColumns) cards.
 */
...
/** Eight Off tableau rule: only a King lands on an empty column; else same-suit descending. */
function canPlaceOnTableau(bottom: Card | undefined, col: readonly Card[]): boolean {
	if (!bottom) return false;
	const t = top(col);
	if (!t) return bottom.rank === KING; // only a King to an empty column
	return bottom.suit === t.suit && bottom.rank === t.rank - 1;
}

/** Max sequence length movable as a supermove. */
function maxSupermove(state: EightOffState, toEmptyColumn: boolean): number {
...
function grabRun(state: EightOffState, pileId: string, cardId: string): readonly Card[] | null {
...
export function makeEightOff(): Game<EightOffState, EightOffMove> {
	const definition: GameDefinition<EightOffState, EightOffMove> = {
		meta: {
			id: 'eightoff',
			name: 'Eight Off',
			blurb: 'Every card in view and eight cells to spare — a gentle, open builder.',
			difficulty: 'easy',
			family: 'builder',
			howTo: [
				'Build the four foundations up by suit, Ace to King.',
				'Build down the tableau in the same suit (8♠ on 9♠).',
				'The eight free cells each hold one card — four start filled.',
				'Only a King may move onto an empty column.',
				'Longer sequences move at once only when enough cells/columns are open.',
				'Double-tap a card to send it straight to a foundation.'
			],
			learnMore: 'https://en.wikipedia.org/wiki/Eight_Off'
		},

		initialState(seed) {
			const deck = shuffledDeck(seed).map((c) => setFaceUp(c, true));
			const tableau: Card[][] = Array.from({ length: COLS }, () => []);
			const freeCells: (Card | null)[] = Array.from({ length: CELLS }, () => null);
			deck.forEach((card, i) => {
				if (i < COLS * 6) tableau[i % COLS].push(card);
				else freeCells[i - COLS * 6] = card; // last 4 cards → cells 0..3
			});
			return {
				freeCells,
		...
		legalMoves(state) {
			...
			state.tableau.forEach((col, ci) => {
				...
				// movable same-suit sequences → other columns (supermove-limited)
				const firstOfRun = (() => {
					let i = col.length - 1;
					while (i > 0 && col[i - 1].suit === col[i].suit && col[i - 1].rank === col[i].rank + 1)
						i--;
					return i;
				})();
	...
	const presenter: GamePresenter<EightOffState, EightOffMove> = {
		layout(): TableLayout {
			const slots = [
				...
				...Array.from({ length: COLS }, (_, i) => ({ pileId: tableauId(i), x: i, y: 2.3 }))
			];
...
export const eightoff = makeEightOff();
